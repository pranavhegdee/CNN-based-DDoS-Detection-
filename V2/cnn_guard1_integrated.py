...
if __name__ == '__main__':
    print("[🔄 INIT] Performing cold-boot file state cleanup...")
    LOCKDOWN_FILE = "/tmp/lockdown.txt"
    if os.path.exists(LOCKDOWN_FILE):
        try:
            os.remove(LOCKDOWN_FILE)
            print("[✅ INIT] Successfully removed stale lockdown state file.")
        except Exception as e:
            print(f"[⚠️ INIT] Could not remove stale lockdown file: {e}")
    
    RSA_QUEUE_FILE = "/tmp/rsa_mitigation_queue.jsonl"
    if os.path.exists(RSA_QUEUE_FILE):
        try:
            os.remove(RSA_QUEUE_FILE)
            print("[✅ INIT] Flushed persistent RSA mitigation queue file successfully.")
        except Exception as e:
            print(f"[⚠️ INIT] Could not delete queue file: {e}")
            
    try:
        requests.post("http://127.0.0.1:8080/mitigate/clear", json={"clear_all": True}, timeout=1.0)
        print("[✅ INIT] Flushed previous switch drop rules.")
    except Exception:
        print("[⚠️ INIT] Controller unreachable during network flush.")

    # 3. CRITICAL: Reset memory structures handling bad RSA signatures
    blocked_ips.clear() 
    recent_attack_calls.clear()
    while True:
        main_loop()