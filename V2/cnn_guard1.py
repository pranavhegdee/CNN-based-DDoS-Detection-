...
def report_telemetry(status, prediction, attack_type="BENIGN", attacker_ip=None, new_blocked_ip=None):
    ...
    try:
        requests.post(f"{DASHBOARD_URL}/api/telemetry", json={
            ...
            "attack_type": attack_type,  # Sends string identity to dashboard UI
            "status": status,
...
def process_packet(packet):
    ...
    if packet.haslayer(IP):
        ...
        if packet.haslayer(TCP):
            ...
            if flags & 0x02: tcp_syn_flags += 1  # SYN
            if flags & 0x10: tcp_ack_flags += 1  # ACK
            if flags & 0x04: tcp_rst_flags += 1  # RST
            if flags & 0x08: tcp_psh_flags += 1  # PSH
            if flags & 0x01: tcp_fin_flags += 1  # FIN
            if flags & 0x20: tcp_urg_flags += 1  # URG
...
def main_loop():
    ...
    # Calculate dominant protocol encoding element
    primary_protocol = 1.0  # Default to TCP
    if icmp_pkts > tcp_pkts and icmp_pkts > udp_pkts:
        primary_protocol = 3.0  # Explicitly ICMP
    elif udp_pkts > tcp_pkts:
        primary_protocol = 2.0  # Explicitly UDP
    
    # Build raw vector (aligned with training pipeline dimensions)
    raw_vector = np.array([[
    ...
    # 3. Dynamic input sizing and padding to match MinMaxScaler fit constraints
    ...
    # 4. Standardize and reshape matrix to 8x8 image
    ...
    # 5. Model Inference Evaluation Phase
    probabilities = model.predict(traffic_image, verbose=0)[0]
    predicted_class_idx = np.argmax(probabilities)
    confidence = probabilities[predicted_class_idx]
    
    # Translate class index back into plain text string label
    attack_class = label_encoder.inverse_transform([predicted_class_idx])[0]
    
    # --- CRITICAL SAFETY OVERRIDE GUARDRAIL ---
    # Slowloris relies strictly on TCP connections. If pure ICMP/Ping is observed, force Benign.
    if attack_class == "SLOWLORIS" and tcp_pkts == 0:
        attack_class = "BENIGN"
        confidence = 0.0
    
    # 6. Action Execution Map and Telemetry Dispatch
    if attack_class != "BENIGN" and confidence > 0.45:
        ...
        # Volumetric Fallbacks
        if pkt_count > 1500 and primary_protocol == 2.0:
            attacker_ip = Counter(src_ips).most_common(1)[0][0] if src_ips else "0.0.0.0"
            new_blocked = execute_mitigation_playbook("UDP_FLOOD", attacker_ip, 1.0)
            report_telemetry("ATTACK", 1.0, attack_type="UDP_FLOOD", attacker_ip=attacker_ip, new_blocked_ip=new_blocked)
        elif pkt_count > 1500 and primary_protocol == 3.0:
            attacker_ip = Counter(src_ips).most_common(1)[0][0] if src_ips else "0.0.0.0"
            new_blocked = execute_mitigation_playbook("ICMP_FLOOD", attacker_ip, 1.0)
            report_telemetry("ATTACK", 1.0, attack_type="ICMP_FLOOD", attacker_ip=attacker_ip, new_blocked_ip=new_blocked)
        else:
            if os.path.exists("/tmp/lockdown.txt"):
                print(f"[🛡️ LOCKDOWN ACTIVE] Fabric load stable ({pkt_count} Pkt/s). Asymmetric checkpoint remains armed.")
